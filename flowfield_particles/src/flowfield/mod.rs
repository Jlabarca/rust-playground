...
pub const NB_PARTICLES: usize = 500;
...
pub struct Flowfield {
	...
	time : f64,
	perlin: Perlin,
	par: [particles::Particle; NB_PARTICLES],
	state: State,
	mode: Mode,
	// slc: Vec<[particles::Particle]>
}
...
impl Flowfield {
	pub fn new(_seed: u32) -> Flowfield {
		...
		let mut parti = arr![particles::Particle::new(); 500];
		let mut slc = Vec::new();

		let bn_part_by_thread = NB_PARTICLES / 4;

		for i in 0..4 {
			slc.push(&parti[i*bn_part_by_thread..(i+1)*bn_part_by_thread]);
		}
		Flowfield {
			field : field,
	...
	fn _update_particles(&mut self) {
	    ...
	    //         .map(|chunk| scope.spawn(move |_| {println!("{:?}", chunk.len())}).to_vec();
...
impl<'a> IntoIterator for &'a Flowfield {
    type Item = &'a particles::Particle;
    type IntoIter = ::std::slice::Iter<'a, particles::Particle>;
    fn into_iter(self) -> ::std::slice::Iter<'a, particles::Particle> {
        self.par.iter()
    }
}

fn worker(slice : &mut [particles::Particle]) {
